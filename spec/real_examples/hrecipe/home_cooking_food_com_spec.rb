...
describe Hangry do
  ...
  context "homecooking.about.com recipe" do
    before(:all) do
      @html = File.read("spec/fixtures/hrecipe/homecooking.about.com.html")
      @parsed = Hangry.parse(@html)
    end
    subject { @parsed }

    it "should use a non-standard parser" do
      expect(Hangry::ParserSelector.new(@html).parser).to be_an_instance_of(Hangry::Parsers::NonStandard::HomeCookingParser)
    end
    ...
    its(:image_url) { should == "http://0.tqn.com/d/homecooking/1/0/G/C/1/blmuff23.jpg" }
    its(:ingredients) {
      should == [
        "Cupcakes/Cake:", "1-3/4 cups all-purpose flour", "2 teaspoons baking powder",
        "1/2 teaspoon salt", "6 Tablespoons butter, softened", "1 cup sugar", "2 large eggs, lightly beaten",
        "1-1/2 teaspoons grated key lime zest", "1 Tablespoon fresh key lime juice", "2/3 cup buttermilk",
        "Glaze:", "1 cup powdered sugar", "1/8 cup fresh key lime juice"
      ]
    }
...
Cut into 18 pieces to serve.Yield: 18 servings
      EOS
