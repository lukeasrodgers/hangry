...
describe Hangry do
  ...
  context "standard food network recipe" do
    before(:all) do
      @html = File.read("spec/fixtures/schema_org/food_network_schema_org.html")
      @parsed = Hangry.parse(@html)
    end
    subject { @parsed }

    it "should use a non-standard parser" do
      expect(Hangry::ParserSelector.new(@html).parser).to be_an_instance_of(Hangry::Parsers::NonStandard::FoodNetworkParser)
    end
    ...
    its(:description) { should == "Get this all-star, easy-to-follow Spinach and Mushroom Stuffed Chicken Breasts recipe from Rachael Ray" }
...
Recipe courtesy of Rachael Ray
  ...
  context "food network recipe with some blank ingredients" do
    before(:all) do
      @html = File.read("spec/fixtures/schema_org/food_network_with_blank_ingredients.html")
      @parsed = Hangry.parse(@html)
    end
    subject { @parsed }
    ...
    its(:description) { should == 'Get this all-star, easy-to-follow Easter Bunny Cake recipe from Food Network Kitchen' }
    ...
    its(:ingredients) do
      should == [
        "6 cups confectioners' sugar", "3 sticks (12 ounces) unsalted butter, at room temperature", "Pinch fine salt",
        "1 tablespoon pure vanilla extract", "2 to 3 tablespoons milk",
        "2 baked 9-inch round cake layers (your favorite recipe or an 18.25-ounce boxed cake mix)",
        "1 1/4 cups sweetened flaked coconut", "2 store-bought biscotti", "1 tube pink decorating icing",
        "2 black jelly beans", "2 marshmallows", "1 white jelly bean, halved lengthwise",
        "1 pink jelly bean", "1 black licorice wheel, such as Haribo"
      ]
    end
